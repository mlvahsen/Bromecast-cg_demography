...
tibble(lower = c(low_dens_pred[1,], high_dens_pred[1,]),
  ...
  geom_vline(aes(xintercept = x_peak_s_high_sc), color = colors[1], linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = x_peak_s_low_sc), color = colors[2], linewidth = 1, linetype = "dashed") +
...
tibble(lower = c(low_dens_pred[1,], high_dens_pred[1,]),
  ...
  labs(y = "ln(fecundity)",
       ...
       color = "") +
  geom_vline(aes(xintercept = 0), color = "gray27", linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = x_peak_high_sc), color = colors[1], linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = x_peak_low_sc), color = colors[2], linewidth = 1, linetype = "dashed") +
  scale_color_manual(values = colors) +
  scale_fill_manual(values = colors)  -> fecundity_sub

## Fitness plot ####
fitness_pred_1 <- plogis(dens1_s) * exp(dens2)
fitness_pred_2 <- plogis(dens2_s) * exp(dens1)
...
fit_data %>% 
  ggplot(aes(x = clim_dist, y = log(fitness), color = density, group = density)) +
  geom_line(linewidth = 1.5) +
  geom_ribbon(aes(ymin = log(lower), ymax = log(upper), fill = density), alpha = 0.2, color = NA) +
  theme_classic(base_size = 16) +
  labs(y = "ln(fitness)",
       x = "climate mismatch",
       fill = "",
       color = "") +
  geom_vline(aes(xintercept = 0), color = "gray27", linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = x_high_peak), color = colors[1], linewidth = 1, linetype = "dashed") +
  geom_vline(aes(xintercept = x_low_peak), color = colors[2], linewidth = 1, linetype = "dashed") +
  scale_color_manual(values = colors) +
...
## Full plot ####
png("figs/Fig6_densxlocaladapt.png", height = 9.5, width = 5.5, res = 300, units = "in")
