data {
  ...
  matrix[N, P] X;
  array[N] int<lower=0> seed_count;
}
...
model {
  ...
  for (n in 1:N) {
    ...
    seed_count[n] ~ neg_binomial_2(exp(log_mu), phi);
  }
}
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    seed_count_pred[n] = neg_binomial_2_rng(exp(log_mu), phi);
