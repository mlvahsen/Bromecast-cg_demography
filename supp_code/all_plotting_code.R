...
draws %>% 
  ...
  gather(key = parameter, value = value, sigma_genotype:sigma_genotype_slope_climdist2) %>% 
  group_by(parameter) %>% 
...
abline(v = mean(cg_model_fecun$seed_count), col = "red", lwd = 2)

draws %>% 
  dplyr::select(contains("seed_count_pred")) %>% 
  apply(MARGIN = 1, FUN = sd) %>% hist(main = "pp check (sd)", xlab = "predicted sd")
abline(v = sd(cg_model_fecun$seed_count), col = "red", lwd = 2)

draws %>% 
  dplyr::select(contains("seed_count_pred")) %>% 
  apply(MARGIN = 1, FUN = max) %>% hist(main = "pp check (max)", xlab = "predicted mean")
abline(v = max(cg_model_fecun$seed_count), col = "red", lwd = 2)

## Marginal effect of neighbors ####
seq_sc_nb <- seq(min(cg_model_fecun$sqrt_new_neighbors_sc),
                 max(cg_model_fecun$sqrt_new_neighbors_sc),
                 length.out = 10)
seq_nb <- seq(min(cg_model_fecun$new_neighbors),
              max(cg_model_fecun$new_neighbors),
              length.out = 10)
...
tibble(lower = sum_stat[1,],
  ...
  geom_point(data = cg_model_fecun, alpha = 0.2) +
...
seq_sc_clim <- seq(min(cg_model_fecun$clim_dist_sc),
                   max(cg_model_fecun$clim_dist_sc), length.out = 10)
seq_clim <- seq(min(cg_model_fecun$clim_dist),
                max(cg_model_fecun$clim_dist), length.out = 10)
for (j in 1:90){
...
png("figs/Fig6_int.png", height = 7, width = 6, res = 300, units = "in")
int_nbvwc_plot
dev.off()
...
tibble(lower = plogis(sum_stats_dens[1,]),
  ...
  geom_jitter(data = cg_model, alpha = 0.3, height = 0.05, size = 0.2, shape = 16) +
...
seq_sc_vwcs <- seq(min(cg_model$vwc_avg_sc),
                   max(cg_model$vwc_avg_sc),
                   length.out = 10)
seq_vwcs <- seq(min(cg_model$vwc_avg),
...
seq_sc_clims <- seq(min(cg_model_fecun$clim_dist_sc),
                    max(cg_model_fecun$clim_dist_sc), length.out = 10)
