...
cg_model %>%
  mutate(plot_unique = paste(site, block, plot, year, sep = "_")) -> cg_model
...
cg_model %>% 
  mutate(new_neighbors = case_when(density == "lo" & possible_neighbors == 3 ~ prop_survived + neighbors,
                                   ...
                                   density == "lo" & possible_neighbors == 0 ~ prop_survived * 4 + neighbors,
                                   density == "hi" & site != "WI" & year == "2023" ~ prop_survived * (80-possible_neighbors) + neighbors,
                                   density == "hi" & site == "WI" & year == "2023" ~ prop_survived * (90-possible_neighbors) + neighbors,
                                   density == "hi" & year == "2022" ~ prop_survived * (100-possible_neighbors) + neighbors,
...
source("supp_code/climate_data.R")
...
site_info <- cbind(vwc_sub,
  ...
  mutate(site = case_when(site == "Boise High" ~ "BA",
                          site == "Boise Low" ~ "WI",
                          site == "Cheyenne" ~ "CH",
                          site == "Sheep Station" ~ "SS"),
         gravel = tolower(gravel)) %>% 
